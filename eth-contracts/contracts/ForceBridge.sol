...
import {StandardGwERC20} from './token/token/StandardGwERC20.sol';
...
contract ForceBridge {
  ...
  // create standard Godwoken ERC20 token
  function createGwERC20(
    bytes memory l1Address,
    bytes memory tokenArgs
  ) public payable returns (address addr) {
    bytes32 salt = keccak256(l1Address);
    addr = Create2.deploy(
      msg.value,
      salt,
